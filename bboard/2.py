...
class BBoard():
    ...
    def __init__(self, board=None, move=0):
        if board is None:
            self.board = np.array(deepcopy(board))
        else:
            self.board = np.array([
                        ['br', 'bn', 'bb', 'bq', 'bk', 'bb', 'bn', 'br'],
                        ['bp', 'bp', 'bp', 'bp', 'bp', 'bp', 'bp', 'bp'],
    ...
    def isValidMove(self, piece, piece_pos, end_pos, optional_enpassant=False):
        ...
        if piece[1] == "b":
    # Check for diagonal
            if np.abs(end_pos[0]-piece_pos[0]) == np.abs(end_pos[1]-piece_pos[1]):
                return self.isValidDiagRow(piece_pos[0], piece_pos[1], end_pos[0], end_pos[1])
            return False

        if piece[1] == "n":
            if np.abs(end_pos[0]-piece_pos[0]) == 2 and np.abs(end_pos[1]-piece_pos[1]) == 1:
                return True
            elif np.abs(end_pos[0]-piece_pos[0]) == 1 and np.abs(end_pos[1]-piece_pos[1]) == 2:
                return True
            return False
        ...
        if piece[1] == "k":
            if np.abs(end_pos[0]-piece_pos[0]) == np.abs(end_pos[1]-piece_pos[1]):
                if np.abs(end_pos[0]-piece_pos[0]) == 1 and np.abs(end_pos[1]-piece_pos[1]) == 1:
                    return True
        ...
        if piece[1] == "q":
            if piece_pos[0] == end_pos[0] and piece_pos[1] != end_pos[1]:
                ...
                return self.isValidDiagRow(piece_pos[0], piece_pos[1], end_pos[0], end_pos[1])
            elif np.abs(end_pos[0]-piece_pos[0]) == np.abs(end_pos[1]-piece_pos[1]):
                return self.isValidDiagRow(piece_pos[0], piece_pos[1], end_pos[0], end_pos[1])
            return False
    ...
    def isInCheck(self, optional_return_both=False):
        ...
        for x in range(0, len(white_pieces)):
            if self.isValidMove(white_pieces[x][0], white_pieces[x][1], black_pieces[0][1]) == True:
        ...
        for x in black_pieces:
            if self.isValidMove(x[0], x[1], white_pieces[0][1]) == True:
            ...
            for piece in white_pieces:
                for row in range(0, 8):
                    for column in range(0, 8):
                        if self.makeMove((piece[1][0], piece[1][1], piece[0]), row, column, optional_return=True) == True:
                            moves_list.append([(piece[1][0], piece[1][1], piece[0]), (row,column)])
        elif COLOUR == "BLACK":
            black_pieces = self.black_pieces
